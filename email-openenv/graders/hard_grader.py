...
def _calculate_semantic_similarity(reply: str, email_text: str) -> float:
    if not reply or not email_text:
        return 0.0
    
    emb1 = _embedding_model.encode(reply, convert_to_tensor=True)
    emb2 = _embedding_model.encode(email_text, convert_to_tensor=True)
...
def grade_hard(action, email):
    ...
    if should_reply and reply_text.strip():
        relevance_score = _calculate_semantic_similarity(reply_text, email_text)
