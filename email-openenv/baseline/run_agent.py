...
Uses Gemini API to generate task-specific actions for email processing.
...
def generate_action(client: genai.Client, observation, task: str) -> dict:
    """Generate an action using Gemini API. Returns action or empty default."""
...
def run_agent(task: Literal["easy", "medium", "hard"] = "easy", max_steps: int = 5):
    ...
    if not api_key:
        raise ValueError(
            ...
            "Please either:\n"
            "  1. Set environment variable: export GEMINI_API_KEY='AI...'\n"
            "  2. Create .env file with: GEMINI_API_KEY=AI...\n"
